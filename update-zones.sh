...
if [ -n "$IP_8" ]; then
  ...
  (timeout -v 30 ./upload-minikube8-zone-generator.sh 2>&1) || true
fi
if [ -n "$IP_9" ]; then
  echo Uploading to minikube9
  (timeout -v 30 ./upload-minikube9-zone-generator.sh 2>&1) || true
fi
if [ -n "$IP_10" ]; then
  echo Uploading to minikube10
  (timeout -v 30 ./upload-minikube10-zone-generator.sh 2>&1) || true
fi
